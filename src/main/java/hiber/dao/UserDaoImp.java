package hiber.dao;

import hiber.model.Car;
import hiber.model.User;
import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

import javax.persistence.TypedQuery;
import java.util.List;

@Repository
public class UserDaoImp implements UserDao {

   @Autowired
   private SessionFactory sessionFactory;

   @Override
   public void add(User user) {
      sessionFactory.getCurrentSession().save(user);
   }

   @Override
   @SuppressWarnings("unchecked")
   public List<User> listUsers() {
      TypedQuery<User> query=sessionFactory.getCurrentSession().createQuery("from User");
      return query.getResultList();
   }

   @Override
   @SuppressWarnings("unchecked")
   public User getCar(String model, int series) {
      TypedQuery<Car> carTypedQuery = sessionFactory.openSession().createQuery(
              "from Car where model = :carModel and series = :series"
      );
      carTypedQuery.setParameter("carModel", model);
      carTypedQuery.setParameter("series", series);
      Car car = carTypedQuery.getSingleResult();

      TypedQuery<User> userTypedQuery = sessionFactory.openSession().createQuery(
              "from User where car_series = :carSeries"
      );
      userTypedQuery.setParameter("carSeries", car.getSeries());
      return userTypedQuery.getSingleResult();
   }

}
